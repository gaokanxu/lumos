...
```bash
lumos-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
lumos-tokens transaction-log --output-path transactions.csv
```
...
```bash
lumos-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
lumos-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
lumos-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
lumos-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
lumos-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `lumos-tokens balances`.
...
```bash
lumos-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
