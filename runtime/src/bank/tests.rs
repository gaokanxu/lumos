#![cfg(test)]
#[allow(deprecated)]
use lumos_sdk::sysvar::fees::Fees;
use {
    super::{
...
fn create_genesis_config_no_tx_fee_no_rent(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let genesis_config_info = lumos_runtime::genesis_utils::create_genesis_config(lamports);
    (
...
fn create_genesis_config_no_tx_fee(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let (mut genesis_config, mint_keypair) =
        lumos_sdk::genesis_config::create_genesis_config(lamports);
...
fn test_race_register_tick_freeze() {
    ...
    let p = lumos_sdk::pubkey::new_rand();
...
fn test_rent_distribution() {
    ...
    let bootstrap_validator_pubkey = lumos_sdk::pubkey::new_rand();
    ...
    let validator_1_pubkey = lumos_sdk::pubkey::new_rand();
    ...
    let validator_2_pubkey = lumos_sdk::pubkey::new_rand();
    ...
    let validator_3_pubkey = lumos_sdk::pubkey::new_rand();
...
fn test_rent_exempt_executable_account() {
    ...
    let account_pubkey = lumos_sdk::pubkey::new_rand();
    let account_balance = 1;
    let mut account = AccountSharedData::new(account_balance, 0, &lumos_sdk::pubkey::new_rand());
...
fn test_rent_eager_collect_rent_in_partition(should_collect_rent: bool) {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != lumos_sdk::feature_set::skip_rent_rewrites::id()
            && (!should_collect_rent
                || feature_id != lumos_sdk::feature_set::disable_rent_fees_collection::id())
        {
            activate_feature(&mut genesis_config, feature_id);
        }
    }

    let zero_lamport_pubkey = lumos_sdk::pubkey::new_rand();
    let rent_due_pubkey = lumos_sdk::pubkey::new_rand();
    let rent_exempt_pubkey = lumos_sdk::pubkey::new_rand();
    let mut bank = Arc::new(Bank::new_for_tests(&genesis_config));
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let vote_id = lumos_sdk::pubkey::new_rand();
    let mut vote_account =
        vote_state::create_account(&vote_id, &lumos_sdk::pubkey::new_rand(), 0, 100);
    let stake_id1 = lumos_sdk::pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = lumos_sdk::pubkey::new_rand();
...
fn test_transfer_to_sysvar() {
    ...
    let normal_pubkey = lumos_sdk::pubkey::new_rand();
...
fn test_bank_tx_fee() {
    ...
    let leader = lumos_sdk::pubkey::new_rand();
    let GenesisConfigInfo {
        mut genesis_config,
        mint_keypair,
        ..
...
fn test_readonly_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &lumos_sdk::pubkey::new_rand(), 0);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;

    let vote_pubkey0 = lumos_sdk::pubkey::new_rand();
    let vote_pubkey1 = lumos_sdk::pubkey::new_rand();
    let vote_pubkey2 = lumos_sdk::pubkey::new_rand();
...
fn test_readonly_relaxed_locks() {
    ...
    let key3 = lumos_sdk::pubkey::new_rand();
...
fn test_bank_update_sysvar_account() {
    ...
    for pass in 0..5 {
        ...
        let dummy_clock_id = lumos_sdk::pubkey::new_rand();
...
fn test_bank_vote_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &lumos_sdk::pubkey::new_rand(), 1);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
...
fn test_bank_cloned_stake_delegations() {
    ...
    let (vote_balance, stake_balance) = {
        ...
        let minimum_delegation = lumos_stake_program::get_minimum_delegation(&bank.feature_set);
        (
...
fn test_is_delta_with_no_committables() {
    ...
    assert_eq!(
        bank.transfer(10_001, &mint_keypair, &lumos_sdk::pubkey::new_rand()),
        Err(TransactionError::InstructionError(
            0,
            SystemError::ResultWithNegativeLamports.into(),
        ))
...
fn test_bank_get_program_accounts() {
    ...
    assert!(
        genesis_accounts
            .iter()
            .any(|(pubkey, _, _)| lumos_sdk::sysvar::is_sysvar_id(pubkey)),
        "no sysvars found"
    );
...
fn test_add_duplicate_static_program() {
    ...
    bank.add_mockup_builtin(lumos_vote_program::id(), MockBuiltin::vm);
    let bank = bank_forks
        .write()
        .unwrap()
        .insert(bank)
    ...
    let vote_loader_account = bank.get_account(&lumos_vote_program::id()).unwrap();
    let new_vote_loader_account = bank.get_account(&lumos_vote_program::id()).unwrap();
...
fn test_transaction_with_duplicate_accounts_in_instruction() {
    ...
    let from_pubkey = lumos_sdk::pubkey::new_rand();
    let to_pubkey = lumos_sdk::pubkey::new_rand();
...
fn test_account_ids_after_program_ids() {
    ...
    let instruction = Instruction::new_with_bincode(lumos_vote_program::id(), &10, account_metas);
    let mut tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    tx.message.account_keys.push(lumos_sdk::pubkey::new_rand());
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    let account = bank.get_account(&lumos_vote_program::id()).unwrap();
...
fn test_program_id_as_payer() {
    ...
    tx.message.account_keys.push(lumos_vote_program::id());
...
fn test_bank_hash_consistency() {
    ...
    activate_feature(
        &mut genesis_config,
        lumos_sdk::feature_set::set_exempt_rent_epoch_max::id(),
    );
...
fn test_add_builtin_loader_no_overwrite() {
    let slot = 123;
    let loader_id = lumos_sdk::pubkey::new_rand();
...
fn test_bank_load_program() {
    ...
    let programdata_key = lumos_sdk::pubkey::new_rand();
        ...
        ) + std::marker::Send,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &lumos_sdk::pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
    ...
    let all_pubkeys: Vec<Pubkey> = std::iter::repeat_with(lumos_sdk::pubkey::new_rand)
        .take(total_pubkeys)
        .collect();
...
fn test_get_inflation_start_slot_devnet_testnet() {
    let GenesisConfigInfo {
        mut genesis_config, ..
    } = create_genesis_config_with_leader(42, &lumos_sdk::pubkey::new_rand(), 42);
    genesis_config
        .accounts
...
fn test_stake_vote_account_validity() {
    ...
    // https://github.com/lumos-labs/lumos/pull/24200#discussion_r849935444
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        lumos_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    let message_arr = b"hello";
    let instruction =
        lumos_sdk::ed25519_instruction::new_ed25519_instruction(&privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_an_empty_instruction_without_program() {
    ...
    let destination = lumos_sdk::pubkey::new_rand();
...
fn test_invalid_rent_state_changes_fee_payer() {
    ...
    genesis_config.fee_rate_governor = FeeRateGovernor::new(
        lumos_sdk::fee_calculator::DEFAULT_TARGET_LAMPORTS_PER_SIGNATURE,
        lumos_sdk::fee_calculator::DEFAULT_TARGET_SIGNATURES_PER_SLOT,
    );
...
fn test_rent_state_incinerator() {
    ...
    for amount in [rent_exempt_minimum - 1, rent_exempt_minimum] {
        bank.transfer(amount, &mint_keypair, &lumos_sdk::incinerator::id())
            .unwrap();
    }
}
...
fn test_resize_and_rent() {
    ...
    let rent_paying_pubkey = lumos_sdk::pubkey::new_rand();
...
fn test_accounts_data_size_and_rent_collection(should_collect_rent: bool) {
    ...
    if should_collect_rent {
        genesis_config
            .accounts
            .remove(&lumos_sdk::feature_set::disable_rent_fees_collection::id());
    }
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            lumos_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_is_in_slot_hashes_history() {
    use lumos_sdk::slot_hashes::MAX_ENTRIES;
...
fn test_program_execution_restricted_for_stake_account_in_reward_period() {
    use lumos_sdk::transaction::TransactionError::ProgramExecutionTemporarilyRestricted;
