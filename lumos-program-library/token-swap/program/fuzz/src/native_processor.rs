...
impl program_stubs::SyscallStubs for TestSyscallStubs {
    fn sol_invoke_signed(
        ...
        if !account_infos.iter().any(|x| *x.key == lpl_token::id()) {
            ...
            for account_info in account_infos.iter() {
                if meta.pubkey == *account_info.key {
                    ...
                    for seeds in signers_seeds.iter() {
                        let signer =
                            Pubkey::create_program_address(seeds, &lpl_token_swap::id()).unwrap();
...
pub fn do_process_instruction(instruction: Instruction, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    let account_infos = account_data
        ...
        .collect::<Vec<_>>();
    let res = if instruction.program_id == lpl_token_swap::id() {
        lpl_token_swap::processor::Processor::process(
            &instruction.program_id,
            &account_infos,
            &instruction.data,
        )
