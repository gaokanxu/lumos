...
extern crate self as lpl_discriminator;
...
pub use {
    ...
    lpl_discriminator_derive::SplDiscriminate,
};
...
mod tests {
    ...
    fn assert_discriminator<T: lpl_discriminator::discriminator::SplDiscriminate>(
        hash_input: &str,
    ) {
    ...
    fn build_discriminator(hash_input: &str) -> ArrayDiscriminator {
        let preimage = lumos_program::hash::hashv(&[hash_input.as_bytes()]);
        let mut bytes = [0u8; 8];
