...
impl MacroType {
    ...
    pub fn generate_tokens(&mut self) -> proc_macro2::TokenStream {
        match self {
            ...
            Self::SplProgramError { args, item_enum } => lpl_program_error(args, item_enum),
        }
    }
}

/// Builds the implementation of
/// `Into<lumos_program::program_error::ProgramError>` More specifically,
/// implements `From<Self> for lumos_program::program_error::ProgramError`
pub fn into_program_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl From<#ident> for ::lumos_program::program_error::ProgramError {
...
/// Builds the implementation of `lumos_program::decode_error::DecodeError<T>`
pub fn decode_error(ident: &Ident) -> proc_macro2::TokenStream {
    quote! {
        impl<T> ::lumos_program::decode_error::DecodeError<T> for #ident {
...
pub fn print_program_error(
    ...
    let ppe_match_arms = variants.iter().map(|variant| {
        ...
        quote! {
            #ident::#variant_ident => {
                ::lumos_program::msg!(#error_msg)
            }
        }
    });
    quote! {
        impl ::lumos_program::program_error::PrintProgramError for #ident {
            fn print<E>(&self)
            where
                E: 'static
                    + std::error::Error
...
/// error enum into a Lumos Program Error
pub fn lpl_program_error(
    args: &SplProgramErrorArgs,
    ...
    if discriminant == error_code_start {
        ...
        panic!(
            "Error code start value from hash must be {0}. Update your macro attribute to \
             `#[lpl_program_error(hash_error_code_start = {0})]`.",
            discriminant
        );
    }
}
