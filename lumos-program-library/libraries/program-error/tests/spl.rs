...
#[lpl_program_error(hash_error_code_start = 2_056_342_880)]
...
fn test_library_error_codes() {
    fn get_error_code_check(hash_input: &str) -> u32 {
        let mut nonce: u32 = 0;
        loop {
            let hash = lumos_program::hash::hashv(&[hash_input.as_bytes(), &nonce.to_le_bytes()]);
            let mut bytes = [0u8; 4];
    ...
    assert_eq!(
        ...
        get_error_code_check("lpl_program_error:ExampleLibraryError"),
    );
    assert_eq!(
