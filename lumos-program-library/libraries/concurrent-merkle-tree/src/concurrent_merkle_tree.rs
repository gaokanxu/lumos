    ...
    ConcurrentMerkleTree<MAX_DEPTH, MAX_BUFFER_SIZE>
{
    ...
    pub fn initialize_with_root(
        ...
        if args.root != recompute(args.rightmost_leaf, &proof, args.index) {
            lumos_logging!("Proof failed to verify");
    ...
    pub fn get_change_log(&self) -> Box<ChangeLog<MAX_DEPTH>> {
        if !self.is_initialized() {
            lumos_logging!("Tree is not initialized, returning default change log");
        ...
        let (changelog_index, use_full_buffer) = match self.find_root_in_changelog(current_root) {
            ...
            None => {
                if allow_inferred_proof {
                    lumos_logging!("Failed to find root in change log -> replaying full buffer");
                    (
    ...
    pub fn check_valid_proof(
        ...
        if !self.is_initialized() {
            lumos_logging!("Tree is not initialized, returning false");
            return false;
        }
        if check_leaf_index(leaf_index, MAX_DEPTH).is_err() {
            lumos_logging!("Leaf index out of bounds for max_depth");
            return false;
        }
    ...
    fn try_apply_proof(
        ...
        lumos_logging!("Active Index: {}", self.active_index);
        lumos_logging!("Rightmost Index: {}", self.rightmost_proof.index);
        lumos_logging!("Buffer Size: {}", self.buffer_size);
        lumos_logging!("Leaf Index: {}", leaf_index);
        let valid_root =
    ...
    fn update_buffers_from_proof(&mut self, start: Node, proof: &[Node], index: u32) -> Node {
        ...
        if self.rightmost_proof.index < (1 << MAX_DEPTH) {
            if index < self.rightmost_proof.index {
                ...
                lumos_logging!("Appending rightmost leaf");
