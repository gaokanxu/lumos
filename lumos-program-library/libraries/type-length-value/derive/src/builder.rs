...
impl From<&SplBorshVariableLenPackBuilder> for TokenStream {
    fn from(builder: &SplBorshVariableLenPackBuilder) -> Self {
        ...
        quote! {
            impl #generics lpl_type_length_value::variable_len_pack::VariableLenPack for #ident #generics #where_clause {
                fn pack_into_slice(&self, dst: &mut [u8]) -> Result<(), lpl_type_length_value::lumos_program::program_error::ProgramError> {
                    borsh::to_writer(&mut dst[..], self).map_err(Into::into)
                }

                fn unpack_from_slice(src: &[u8]) -> Result<Self, lpl_type_length_value::lumos_program::program_error::ProgramError> {
                    lumos_program::borsh1::try_from_slice_unchecked(src).map_err(Into::into)
                }

                fn get_packed_len(&self) -> Result<usize, lpl_type_length_value::lumos_program::program_error::ProgramError> {
                    lumos_program::borsh1::get_instance_packed_len(self).map_err(Into::into)
                }
            }
        }
    }
