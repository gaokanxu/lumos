...
pub(crate) mod inline_mpl_token_metadata {
    use {borsh::BorshDeserialize, lumos_program::pubkey::Pubkey};
...
fn process_accept_offer(
    ...
    let is_native = *taker_src_mint.key == lpl_token::native_mint::id();
    if is_native {
    ...
    let maker_src_token_account: lpl_token::state::Account =
        lpl_token::state::Account::unpack(&maker_src_account.data.borrow())?;
    ...
    assert_keys_equal(lpl_token::id(), *token_program_info.key)?;
    ...
    if *taker_src_mint.key == lpl_token::native_mint::id() {
        ...
        Some(creators) => {
            for creator in creators {
                ...
                if !is_native {
                    ...
                    if !seeds.is_empty() {
                        msg!("Maker cannot pay with native LUM");
                        return Err(ProgramError::InvalidAccountData);
                    }
                    match system_program_info {
                        Some(sys_program_info) => {
