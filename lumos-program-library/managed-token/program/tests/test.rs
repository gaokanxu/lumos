use {
    lumos_program::program_option::COption,
    lumos_program_test::*,
    lumos_sdk::{
    ...
    lpl_managed_token::instruction::*,
    lpl_token::state::Account as TokenAccount,
};
...
fn lpl_managed_token_test() -> ProgramTest {
    ProgramTest::new(
        "lpl_managed_token",
        lpl_managed_token::id(),
        processor!(lpl_managed_token::process_instruction),
    )
}

#[tokio::test]
async fn test_lpl_managed_token_basic() {
    let mut context = lpl_managed_token_test().start_with_context().await;
    ...
    let create_eve =
        create_associated_token_account(&authority.pubkey(), &eve_key, &mint_key, &lpl_token::id());
    process_transaction(lwc, vec![create_eve], vec![&authority])
        .await
        .unwrap();

    // Try transfer the normal way
    let failed_transfer_ix = lpl_token::instruction::transfer(
        &lpl_token::id(),
...
#[tokio::test]
async fn test_lpl_managed_token_with_approve_and_revoke() {
    let mut context = lpl_managed_token_test().start_with_context().await;
    let lwc = &mut context.banks_client;
    let authority = Keypair::new();
    transfer(lwc, &context.payer, &authority.pubkey(), sol(10.0))
...
#[tokio::test]
async fn test_lpl_managed_token_with_delegate_transfer() {
    let mut context = lpl_managed_token_test().start_with_context().await;
    let lwc = &mut context.banks_client;
    let authority = Keypair::new();
    transfer(lwc, &context.payer, &authority.pubkey(), sol(10.0))
