...
use {
    crate::{
        check_program_account, check_lpl_token_program_account, error::TokenError,
    ...
    lpl_pod::bytemuck::{pod_from_bytes, pod_get_packed_len},
    std::{
...
pub fn initialize_mint(
    ...
    check_lpl_token_program_account(token_program_id)?;
...
mod test {
    ...
    macro_rules! test_instruction {
        ($a:ident($($b:tt)*)) => {
            let instruction_v3 = lpl_token::instruction::$a($($b)*).unwrap();
    ...
    fn test_v3_compatibility() {
        let token_program_id = lpl_token::id();
        ...
        // set_authority
        {
            let instruction_v3 = lpl_token::instruction::set_authority(
                &token_program_id,
                &owned_pubkey,
                Some(&new_authority_pubkey),
                lpl_token::instruction::AuthorityType::AccountOwner,
                &owner_pubkey,
                &signer_pubkeys,
            )
            .unwrap();
        ...
        {
            let instruction_v3 =
                lpl_token::instruction::get_account_data_size(&token_program_id, &mint_pubkey)
                    .unwrap();
...
impl<'a> TokenInstruction<'a> {
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        ...
        Ok(match tag {
            26 => {
                let instruction = TransferFeeInstruction::try_from_slice(rest)
    ...
    pub fn process(&self, instruction: &Instruction, account_keys: &AccountKeys) -> Result<(), ProgramError> {
        match self {
            TokenInstruction::TransferFeeExtension(transfer_fee_instruction) => {
                crate::extension::transfer_fee::instruction::parse_transfer_fee_instruction(transfer_fee_instruction, account_keys)?;
            },
            _ => {
                println!("Other instruction matched!");
            }
        }
        Ok(())
    }
    ...
    pub fn pack(&self) -> Vec<u8> {
        match self {
            TokenInstruction::TransferFeeExtension(instruction) => {
                ...
                data.push(26); // 加入标识符
                data.extend_from_slice(&instruction.try_to_vec().expect("Failed to serialize"));
                data
            },
            // 其他变体的序列化
        }
