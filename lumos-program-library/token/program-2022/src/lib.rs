...
//! An ERC20-like Token program for the Lumos blockchain
...
pub use {lumos_program, lumos_zk_token_sdk};
...
pub fn check_zk_token_proof_program_account(zk_token_proof_program_id: &Pubkey) -> ProgramResult {
    if zk_token_proof_program_id != &lumos_zk_token_sdk::zk_token_proof_program::id() {
        return Err(ProgramError::IncorrectProgramId);
    }
    Ok(())
}
