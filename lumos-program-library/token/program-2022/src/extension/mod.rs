...
use {
    ...
    lpl_type_length_value::variable_len_pack::VariableLenPack,
    std::{
        cmp::Ordering,
...
mod test {
    ...
    /// which permits "reallocs" as the Lumos runtime does it
    struct LumosAccountData {
        data: Vec<u8>,
        lamports: u64,
        owner: Pubkey,
    }
    impl LumosAccountData {
        /// Create a new fake lumos account data. The underlying vector is
    ...
    impl GetAccount for LumosAccountData {
    ...
    fn alloc_new_fixed_len_tlv_in_account_info_from_base_size() {
        ...
        *state.base = TEST_POD_MINT;
        ...
        let mut data = LumosAccountData::new(&buffer);
