...
use {
    ...
    lumos_client::rpc_request::TokenAccountsFilter,
    ...
    lpl_token_group_interface::state::TokenGroup,
...
async fn command_set_transfer_fee(
    ...
    let token = token_client_from_config(config, &token_pubkey, Some(decimals))?;
    let maximum_fee = lpl_token::ui_amount_to_amount(maximum_fee, decimals);
    let res = token
        .set_transfer_fee(
            &transfer_fee_authority,
            transfer_fee_basis_points,
...
async fn command_create_account(
    ...
    if immutable_owner {
        if config.program_id == lpl_token::id() {
            return Err(format!(
...
async fn command_transfer(
    ...
    let maybe_transfer_balance =
        ui_amount.map(|ui_amount| lpl_token::ui_amount_to_amount(ui_amount, mint_info.decimals));
    ...
    let transfer_balance = if !config.sign_only {
        ...
        println_display(
            ...
            format!(
                ...
                lpl_token::amount_to_ui_amount(transfer_balance, mint_info.decimals),
                sender,
                recipient
            ),
        );
        ...
        if transfer_balance > sender_balance && confidential_transfer_args.is_none() {
            return Err(format!(
                ...
                lpl_token_2022::amount_to_ui_amount_string_trimmed(
                    sender_balance,
                    mint_info.decimals
                )
            )
    ...
    let maybe_fee =
        ui_fee.map(|ui_amount| lpl_token::ui_amount_to_amount(ui_amount, mint_info.decimals));
...
async fn command_burn(
    ...
    let amount = if let Some(ui_amount) = ui_amount {
        lpl_token::ui_amount_to_amount(ui_amount, mint_info.decimals)
    } else {
        if config.sign_only {
    ...
    println_display(
        ...
        format!(
            "Burn {} tokens\n  Source: {}",
            lpl_token::amount_to_ui_amount(amount, mint_info.decimals),
            account
        ),
    );
...
async fn command_mint(
    ...
    let amount = lpl_token::ui_amount_to_amount(ui_amount, mint_info.decimals);
    let decimals = if use_unchecked_instruction {
        None
    } else {
        Some(mint_info.decimals)
...
async fn command_accounts(
    ...
    let filters = if let Some(token_pubkey) = maybe_token {
        ...
        vec![
            TokenAccountsFilter::ProgramId(lpl_token::id()),
            TokenAccountsFilter::ProgramId(lpl_token_2022::id()),
        ]
    };
    ...
    let amount = if !config.sign_only && instruction_type == ConfidentialInstructionType::Deposit {
        ...
        println_display(
            ...
            format!(
                ...
                lpl_token::amount_to_ui_amount(deposit_amount, mint_info.decimals),
            ),
        );
        ...
        println_display(
            ...
            format!(
                ...
                lpl_token::amount_to_ui_amount(withdraw_amount, mint_info.decimals)
            ),
        );
