...
impl<'a> Config<'a> {
    pub async fn new(
        ...
        let websocket_url = lumos_cli_config::Config::compute_websocket_url(&json_rpc_url);
    ...
    pub async fn new_with_clients_and_ws_url(
        ...
        let default_program_id = lpl_token::id();
    ...
    pub fn fee_payer(&self) -> Result<Arc<dyn Signer>, Error> {
        if let Some(fee_payer) = &self.fee_payer {
            ...
            Err("fee payer is required, please specify a valid fee payer using the --fee-payer argument, \
                 ...
                 a valid config at the default location of ~/.config/lumos/cli/config.yml using the lumos \
