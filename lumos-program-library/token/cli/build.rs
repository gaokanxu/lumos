...
fn main() {
    ...
    if is_debug && build_dependent_programs {
        ...
        let lpl_token_2022_dir = cwd
            .parent()
            .expect("Unable to get parent directory of current working dir")
            .join("program-2022");
        rerun_if_changed(&lpl_token_2022_dir);
        let lpl_token_dir = cwd
            .parent()
            .expect("Unable to get parent directory of current working dir")
            .join("program");
        rerun_if_changed(&lpl_token_dir);
        let lpl_associated_token_account_dir = cwd
            .parent()
            .expect("Unable to get parent directory of current working dir")
            .parent()
            .expect("Unable to get parent directory of current working dir")
            ...
            .join("program");
        rerun_if_changed(&lpl_associated_token_account_dir);

        build_bpf(&lpl_token_dir);
        build_bpf(&lpl_token_2022_dir);
        build_bpf(&lpl_associated_token_account_dir);
    }
