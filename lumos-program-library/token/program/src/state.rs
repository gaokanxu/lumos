...
pub struct Account {
    ...
    /// the value is used by the Processor to ensure that wrapped LUM
...
impl Account {
    ...
    pub fn is_owned_by_system_program_or_incinerator(&self) -> bool {
        lumos_program::system_program::check_id(&self.owner)
            || lumos_program::incinerator::check_id(&self.owner)
    }
}
