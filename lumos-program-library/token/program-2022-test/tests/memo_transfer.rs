...
use {
    ...
    lpl_token_client::token::TokenError as TokenClientError,
    std::sync::Arc,
};
...
async fn test_memo_transfers(
    ...
    let mut memo_ix = lpl_memo::build_memo(&[240, 159, 166, 150], &[]);
    for program_id in [lpl_memo::id(), lpl_memo::v1::id()] {
        ...
        let instructions = vec![
            ...
            lpl_token_2022::instruction::transfer(
                &lpl_token_2022::id(),
                &alice_account,
                &bob_account,
                &alice.pubkey(),
    ...
    let mut ctx = context.lock().await;
    memo_ix.program_id = lpl_memo::v1::id();
    #[allow(deprecated)]
    let instructions = vec![
        memo_ix,
        lpl_token_2022::instruction::transfer(
