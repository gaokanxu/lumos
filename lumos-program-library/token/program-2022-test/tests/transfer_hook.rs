...
use {
    ...
    lumos_program_test::{processor, tokio, ProgramTest},
    lumos_sdk::{
        account::Account,
    ...
    lpl_tlv_account_resolution::{account::ExtraAccountMeta, seeds::Seed},
    lpl_token_2022::{
        error::TokenError,
        extension::{
    ...
    lpl_token_client::token::{ExtensionInitializationParams, TokenError as TokenClientError},
...
fn setup_program_test(program_id: &Pubkey) -> ProgramTest {
    ...
    program_test.add_program(
        "my_transfer_hook",
        *program_id,
        processor!(lpl_transfer_hook_example::processor::process),
    );
    program_test
}
...
fn add_validation_account(program_test: &mut ProgramTest, mint: &Pubkey, program_id: &Pubkey) {
    ...
    program_test.add_account(
        ...
        Account {
            ...
            data: lpl_transfer_hook_example::state::example_data(&extra_account_metas).unwrap(),
            owner: *program_id,
            ..Account::default()
        },
    );
...
async fn fail_transfer_hook_program() {
    ...
    program_test.add_account(
        ...
        Account {
            lamports: 1_000_000_000, // a lot, just to be safe
            data: lpl_transfer_hook_example::state::example_data(&[]).unwrap(),
            owner: program_id,
            ..Account::default()
        },
    );
...
async fn success_transfers_using_onchain_helper() {
    ...
    let account_metas = vec![
        ...
        AccountMeta::new_readonly(lpl_token_2022::id(), false),
