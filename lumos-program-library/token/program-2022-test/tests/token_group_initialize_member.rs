...
use {
    ...
    lpl_pod::bytemuck::pod_from_bytes,
    lpl_token_2022::{error::TokenError, extension::BaseStateWithExtensions, processor::Processor},
    lpl_token_client::token::{ExtensionInitializationParams, TokenError as TokenClientError},
    lpl_token_group_interface::{error::TokenGroupError, state::TokenGroupMember},
    std::sync::Arc,
};
...
async fn fail_without_signatures() {
    ...
    let mut instruction = lpl_token_group_interface::instruction::initialize_member(
        &lpl_token_2022::id(),
