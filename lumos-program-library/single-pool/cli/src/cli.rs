use {
    ...
    lumos_sdk::{pubkey::Pubkey, signer::Signer},
    lpl_single_pool::{self, find_pool_address},
...
pub fn pool_address_from_args(maybe_pool: Option<Pubkey>, maybe_vote: Option<Pubkey>) -> Pubkey {
    if let Some(pool_address) = maybe_pool {
        ...
        find_pool_address(&lpl_single_pool::id(), &vote_account_address)
    } else {
        unreachable!()
    }
}

// all this is because lumos clap v3 utils signer handlers dont work with
