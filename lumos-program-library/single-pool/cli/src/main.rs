...
use {
    ...
    lumos_clap_v3_utils::input_parsers::Amount,
    lumos_client::{
    ...
    lumos_vote_program::{self as vote_program, vote_state::VoteState},
    lpl_single_pool::{
...
async fn command_initialize(config: &Config, command_config: InitializeCli) -> CommandResult {
    ...
    let pool_address = find_pool_address(&lpl_single_pool::id(), &vote_account_address);
    ...
    let mut instructions = lpl_single_pool::instruction::initialize(
        &lpl_single_pool::id(),
        &vote_account_address,
        &payer.pubkey(),
...
async fn command_reactivate_pool_stake(
    ...
    if !command_config.skip_deactivation_check {
        ...
        let pool_stake_address = find_pool_stake_address(&lpl_single_pool::id(), &pool_address);
    ...
    let instruction = lpl_single_pool::instruction::reactivate_pool_stake(
        &lpl_single_pool::id(),
        &vote_account_address,
    );
    let transaction = Transaction::new_signed_with_payer(
...
async fn command_deposit(config: &Config, command_config: DepositCli) -> CommandResult {
    ...
    let provided_pool_address = command_config.pool_address.or_else(|| {
        command_config
            .vote_account_address
            .map(|address| find_pool_address(&lpl_single_pool::id(), &address))
    });
        ...
        quarantine::get_stake_info(config, &stake_account_address).await?
    {
        let derived_pool_address =
            find_pool_address(&lpl_single_pool::id(), &stake.delegation.voter_pubkey);
    ...
    let pool_mint_address = find_pool_mint_address(&lpl_single_pool::id(), &pool_address);
    let token = Token::new(
    ...
    let instructions = lpl_single_pool::instruction::deposit(
        &lpl_single_pool::id(),
        &pool_address,
        &stake_account_address,
...
async fn command_withdraw(config: &Config, command_config: WithdrawCli) -> CommandResult {
    ...
    instructions.extend(lpl_single_pool::instruction::withdraw(
        &lpl_single_pool::id(),
        &pool_address,
        &stake_account_address,
        &stake_authority_address,
...
async fn command_create_metadata(
    ...
    let instruction = lpl_single_pool::instruction::create_token_metadata(
        &lpl_single_pool::id(),
        &pool_address,
        &payer.pubkey(),
    );
...
async fn command_update_metadata(
    ...
    let instruction = lpl_single_pool::instruction::update_token_metadata(
        &lpl_single_pool::id(),
        &vote_account_address,
        &authorized_withdrawer.pubkey(),
...
async fn command_create_stake(config: &Config, command_config: CreateStakeCli) -> CommandResult {
    ...
    let instructions = lpl_single_pool::instruction::create_and_delegate_user_stake(
        &lpl_single_pool::id(),
        &vote_account_address,
        &stake_authority_address,
        &quarantine::get_rent(config).await?,
