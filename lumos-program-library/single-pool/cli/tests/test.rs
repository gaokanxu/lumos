...
use {
    serial_test::serial,
    lumos_cli_config::Config as LumosConfig,
...
async fn setup(initialize: bool) -> Env {
    ...
    lumos_config.save(config_file_path).unwrap();
...
async fn start_validator() -> (TestValidator, Keypair) {
    ...
    test_validator_genesis.add_upgradeable_programs_with_path(&[
        ...
        UpgradeableProgramInfo {
            program_id: lpl_single_pool::id(),
            loader: bpf_loader_upgradeable::id(),
            program_path: PathBuf::from("../../target/deploy/lpl_single_pool.so"),
            upgrade_authority: Pubkey::default(),
        },
    ]);
