        ...
        FeatureProposalInstruction::Propose {
            ...
            invoke_signed(
                &system_instruction::create_account(
                    funder_info.key,
                    mint_info.key,
                    1.max(rent.minimum_balance(lpl_token::state::Mint::get_packed_len())),
                    lpl_token::state::Mint::get_packed_len() as u64,
                    &lpl_token::id(),
                ),
                &[
                    funder_info.clone(),
            ...
            invoke(
                &lpl_token::instruction::set_authority(
                    &lpl_token::id(),
                    acceptance_token_info.key,
                    Some(feature_proposal_info.key),
                    lpl_token::instruction::AuthorityType::CloseAccount,
                    feature_proposal_info.key,
                    &[],
                )?,
                &[
            ...
            match feature_proposal_state {
                FeatureProposal::Pending(acceptance_criteria) => {
                    ...
                    let acceptance_token =
                        lpl_token::state::Account::unpack(&acceptance_token_info.data.borrow())?;
