...
use lumos_program::{program_pack::Pack, pubkey::Pubkey};

lumos_program::declare_id!("Feat1YXHhH6t1juaWF74WLcfv4XoNocjXA6sPWHNgAse");
...
pub fn ui_amount_to_amount(ui_amount: f64) -> u64 {
    (ui_amount * 10_usize.pow(lpl_token::native_mint::DECIMALS as u32) as f64) as u64
}

/// Convert a raw amount to its UI representation (using the decimals field
/// defined in its mint)
pub fn amount_to_ui_amount(amount: u64) -> f64 {
    amount as f64 / 10_usize.pow(lpl_token::native_mint::DECIMALS as u32) as f64
}
