...
use {
    ...
    lpl_feature_proposal::{instruction::*, state::*, *},
};

fn program_test() -> ProgramTest {
    ProgramTest::new(
        "lpl_feature_proposal",
        id(),
...
async fn test_basic() {
    ...
    let mint = banks_client
        .get_packed_account_data::<lpl_token::state::Mint>(mint_address)
        .await
        .unwrap();
    ...
    let distributor_token = banks_client
        .get_packed_account_data::<lpl_token::state::Account>(distributor_token_address)
        .await
        .unwrap();
    ...
    let acceptance_token = banks_client
        .get_packed_account_data::<lpl_token::state::Account>(acceptance_token_address)
        .await
        .unwrap();
