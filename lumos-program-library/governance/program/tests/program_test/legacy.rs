use {
    ...
    lpl_governance::state::{enums::GovernanceAccountType, governance::GovernanceV2},
};
...
pub enum VoteWeightSource {
    ...
    /// required in lumos and/or arweave as a prerequisite
    Snapshot,
}
...
impl From<GovernanceV2> for LegacyGovernanceV1 {
    fn from(governance_v2: GovernanceV2) -> Self {
        ...
        let yes_vote_threshold = match governance_v2.config.community_vote_threshold {
            lpl_governance::state::enums::VoteThreshold::YesVotePercentage(yes_vote_percentage) => {
                yes_vote_percentage
            }
