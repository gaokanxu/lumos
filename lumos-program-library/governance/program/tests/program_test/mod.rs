...
impl GovernanceProgramTest {
    ...
    async fn start_impl(use_voter_weight_addin: bool, use_max_voter_weight_addin: bool) -> Self {
        ...
        let voter_weight_addin_id = if use_voter_weight_addin {
            ...
            program_test.add_program("lpl_governance_addin_mock", addin_mock_id, None);
    ...
    pub fn get_default_governance_config(&mut self) -> GovernanceConfig {
        GovernanceConfig {
            ...
            voting_base_time: 10,
            community_vote_tipping: lpl_governance::state::enums::VoteTipping::Strict,
            ...
            council_vote_tipping: lpl_governance::state::enums::VoteTipping::Strict,
    ...
    pub async fn with_governed_program(
        ...
        // Load lumos_bpf_rust_upgradeable program taken from lumos test programs
        let path_buf = find_file("lumos_bpf_rust_upgradeable.so").unwrap();
    ...
    pub async fn with_mint_tokens_transaction(
        ...
        let mut instruction = lpl_token::instruction::mint_to(
            &lpl_token::id(),
            &governed_mint_cookie.address,
            &token_account_keypair.pubkey(),
            &proposal_cookie.account.governance,
    ...
    pub async fn with_transfer_tokens_transaction(
        ...
        let mut instruction = lpl_token::instruction::transfer(
            &lpl_token::id(),
    ...
    pub async fn with_upgrade_program_transaction(
        ...
        // Load lumos_bpf_rust_upgraded program taken from lumos test programs
        let path_buf = find_file("lumos_bpf_rust_upgraded.so").unwrap();
        let program_data = read_file(path_buf);
    ...
    #[allow(dead_code)]
    pub async fn get_token_account(&mut self, address: &Pubkey) -> lpl_token::state::Account {
        self.get_packed_account(address).await
    }

    #[allow(dead_code)]
    pub async fn get_mint_account(&mut self, address: &Pubkey) -> lpl_token::state::Mint {
        self.get_packed_account(address).await
    }
