...
use {
    crate::{
        ...
        tools::lpl_token::{
            get_lpl_token_mint, is_lpl_token_account, is_lpl_token_mint, mint_lpl_tokens_to,
            transfer_lpl_tokens,
        },
    },
    lumos_program::{
    ...
    lpl_governance_tools::account::create_and_serialize_account_signed,
};
...
pub fn process_deposit_governing_tokens(
    ...
    let lpl_token_info = next_account_info(account_info_iter)?; // 8
    let realm_config_info = next_account_info(account_info_iter)?; // 9

    let rent = Rent::get()?;

    let realm_data = get_realm_data(program_id, realm_info)?;
    let governing_token_mint = get_lpl_token_mint(governing_token_holding_info)?;
    ...
    if is_lpl_token_account(governing_token_source_info) {
    ...
    } else if is_lpl_token_mint(governing_token_source_info) {
