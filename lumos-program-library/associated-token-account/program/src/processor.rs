...
fn process_create_associated_token_account(
    ...
    let lpl_token_mint_info = next_account_info(account_info_iter)?;
    let system_program_info = next_account_info(account_info_iter)?;
    let lpl_token_program_info = next_account_info(account_info_iter)?;
    let lpl_token_program_id = lpl_token_program_info.key;

    let (associated_token_address, bump_seed) = get_associated_token_address_and_bump_seed_internal(
        wallet_account_info.key,
        lpl_token_mint_info.key,
        program_id,
        lpl_token_program_id,
    );
    ...
    if create_mode == CreateMode::Idempotent
        && associated_token_account_info.owner == lpl_token_program_id
    {
        ...
        if let Ok(associated_token_account) = StateWithExtensions::<Account>::unpack(&ata_data) {
            ...
            if associated_token_account.base.mint != *lpl_token_mint_info.key {
    ...
    let associated_token_account_signer_seeds: &[&[_]] = &[
        ...
        &lpl_token_program_id.to_bytes(),
        &lpl_token_mint_info.key.to_bytes(),
        &[bump_seed],
    ];

    let account_len = get_account_len(
        lpl_token_mint_info,
        lpl_token_program_info,
    ...
    invoke(
        ...
        &[
            ...
            lpl_token_program_info.clone(),
        ],
    )?;
    invoke(
        &lpl_token_2022::instruction::initialize_account3(
            lpl_token_program_id,
            associated_token_account_info.key,
            lpl_token_mint_info.key,
            wallet_account_info.key,
...
pub fn process_recover_nested(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    if owner_token_mint_info.owner != lpl_token_program_id {
    ...
    let (amount, decimals) = {
        ...
        if owner_associated_token_account_info.owner != lpl_token_program_id {
        ...
        if nested_associated_token_account_info.owner != lpl_token_program_id {
        ...
        if nested_token_mint_info.owner != lpl_token_program_id {
