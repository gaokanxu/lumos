...
pub fn get_account_len<'a>(
    mint: &AccountInfo<'a>,
    lpl_token_program: &AccountInfo<'a>,
    extension_types: &[ExtensionType],
) -> Result<usize, ProgramError> {
    invoke(
        &lpl_token_2022::instruction::get_account_data_size(
            lpl_token_program.key,
            mint.key,
            extension_types,
        )?,
        &[mint.clone(), lpl_token_program.clone()],
    )?;
    get_return_data()
        .ok_or(ProgramError::InvalidInstructionData)
        .and_then(|(key, data)| {
            if key != *lpl_token_program.key {
