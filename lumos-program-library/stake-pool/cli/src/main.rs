...
#[allow(deprecated)]
use lpl_associated_token_account::create_associated_token_account;
use {
    crate::{
        client::*,
...
const FEES_REFERENCE: &str = "Consider setting a minimal fee. \
                              See https://spl.lumos.com/stake-pool/fees for more \
...
fn send_transaction_no_wait(
    config: &Config,
    transaction: Transaction,
) -> lumos_client::client_error::Result<()> {
    if config.dry_run {
...
fn command_create_pool(
    ...
    let mint_account_balance = config
        .rpc_client
        .get_minimum_balance_for_rent_exemption(lpl_token::state::Mint::LEN)?;
    let pool_fee_account_balance = config
        .rpc_client
        .get_minimum_balance_for_rent_exemption(lpl_token::state::Account::LEN)?;
    ...
    let default_decimals = lpl_token::native_mint::DECIMALS;
    ...
    let mut setup_instructions = vec![
        ...
        system_instruction::create_account(
            ...
            &mint_keypair.pubkey(),
            mint_account_balance,
            lpl_token::state::Mint::LEN as u64,
            &lpl_token::id(),
        ),
    ...
    let initialize_instructions = &[
        ...
        lpl_stake_pool::instruction::initialize(
            &lpl_stake_pool::id(),
            &stake_pool_keypair.pubkey(),
            &config.manager.pubkey(),
...
fn create_token_metadata(
    ...
    let instructions = vec![lpl_stake_pool::instruction::create_token_metadata(
        &lpl_stake_pool::id(),
        stake_pool_address,
        &stake_pool.manager,
...
fn update_token_metadata(
    ...
    let mut signers = vec![config.fee_payer.as_ref(), config.manager.as_ref()];
    let instructions = vec![lpl_stake_pool::instruction::update_token_metadata(
        &lpl_stake_pool::id(),
        stake_pool_address,
        &stake_pool.manager,
        &stake_pool.pool_mint,
...
fn command_vsa_add(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[
            lpl_stake_pool::instruction::add_validator_to_pool_with_vote(
                &lpl_stake_pool::id(),
                &stake_pool,
                stake_pool_address,
                vote_account,
...
fn command_vsa_remove(
    ...
    let instructions = vec![
        ...
        lpl_stake_pool::instruction::remove_validator_from_pool_with_vote(
            &lpl_stake_pool::id(),
            &stake_pool,
            stake_pool_address,
            vote_account,
...
fn command_increase_validator_stake(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[
            lpl_stake_pool::instruction::increase_validator_stake_with_vote(
                &lpl_stake_pool::id(),
                &stake_pool,
                stake_pool_address,
                vote_account,
...
fn command_decrease_validator_stake(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[
            lpl_stake_pool::instruction::decrease_validator_stake_with_vote(
                &lpl_stake_pool::id(),
                &stake_pool,
                stake_pool_address,
                vote_account,
...
fn command_set_preferred_validator(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[lpl_stake_pool::instruction::set_preferred_validator(
            &lpl_stake_pool::id(),
            stake_pool_address,
            &config.staker.pubkey(),
...
fn command_deposit_stake(
    ...
    let mut deposit_instructions =
        if let Some(stake_deposit_authority) = config.funding_authority.as_ref() {
            ...
            lpl_stake_pool::instruction::deposit_stake_with_authority(
                &lpl_stake_pool::id(),
                stake_pool_address,
                &stake_pool.validator_list,
                &stake_deposit_authority.pubkey(),
            ...
            lpl_stake_pool::instruction::deposit_stake(
                &lpl_stake_pool::id(),
                stake_pool_address,
                &stake_pool.validator_list,
                &pool_withdraw_authority,
...
fn command_list(config: &Config, stake_pool_address: &Pubkey) -> CommandResult {
    ...
    let total_pool_tokens =
        lpl_token::amount_to_ui_amount(stake_pool.pool_token_supply, pool_mint.decimals);
...
fn command_update(
    ...
    let (mut update_list_instructions, final_instructions) = if stale_only {
        lpl_stake_pool::instruction::update_stale_stake_pool(
            &lpl_stake_pool::id(),
            &stake_pool,
            &validator_list,
            stake_pool_address,
        ...
        lpl_stake_pool::instruction::update_stake_pool(
            &lpl_stake_pool::id(),
            &stake_pool,
            &validator_list,
            stake_pool_address,
...
fn prepare_withdraw_accounts(
    ...
    if remaining_amount > 0 {
        return Err(format!(
            ...
            lpl_token::amount_to_ui_amount(pool_amount, pool_mint.decimals)
        )
        .into());
    }
...
fn command_withdraw_stake(
    ...
    let stake_pool = get_stake_pool(&config.rpc_client, stake_pool_address)?;
    let pool_mint = get_token_mint(&config.rpc_client, &stake_pool.pool_mint)?;
    let pool_amount = lpl_token::ui_amount_to_amount(pool_amount, pool_mint.decimals);
    ...
    if token_account.amount < pool_amount {
        return Err(format!(
            ...
            lpl_token::amount_to_ui_amount(pool_amount, pool_mint.decimals),
            lpl_token::amount_to_ui_amount(token_account.amount, pool_mint.decimals)
        )
        .into());
    }
    ...
    for withdraw_account in withdraw_accounts {
        ...
        if let Some(vote_address) = withdraw_account.vote_address {
            println!(
                ...
                lpl_token::amount_to_ui_amount(withdraw_account.pool_amount, pool_mint.decimals),
        ...
        instructions.push(lpl_stake_pool::instruction::withdraw_stake(
            &lpl_stake_pool::id(),
            stake_pool_address,
            &stake_pool.validator_list,
            &pool_withdraw_authority,
...
fn command_set_manager(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[lpl_stake_pool::instruction::set_manager(
            &lpl_stake_pool::id(),
            stake_pool_address,
            &config.manager.pubkey(),
            &new_manager_pubkey,
...
fn command_set_staker(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[lpl_stake_pool::instruction::set_staker(
            &lpl_stake_pool::id(),
            stake_pool_address,
            &config.manager.pubkey(),
            new_staker,
...
fn command_set_funding_authority(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[lpl_stake_pool::instruction::set_funding_authority(
            &lpl_stake_pool::id(),
            stake_pool_address,
            &config.manager.pubkey(),
...
fn command_set_fee(
    ...
    let transaction = checked_transaction_with_signers(
        config,
        &[lpl_stake_pool::instruction::set_fee(
            &lpl_stake_pool::id(),
            stake_pool_address,
            &config.manager.pubkey(),
            new_fee,
