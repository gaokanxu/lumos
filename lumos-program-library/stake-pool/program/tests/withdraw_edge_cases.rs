...
use {
    ...
    lumos_program_test::*,
    lumos_sdk::{signature::Signer, transaction::TransactionError},
    lpl_stake_pool::{error::StakePoolError, instruction, state},
    test_case::test_case,
};
...
async fn fail_remove_validator() {
    let (
        ...
        _,
    ) = setup_for_withdraw(lpl_token::id(), STAKE_ACCOUNT_RENT_EXEMPTION).await;
