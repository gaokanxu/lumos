...
use {
    ...
    lpl_stake_pool::{error, id, instruction, state, MINIMUM_RESERVE_LAMPORTS},
    lpl_token_2022::extension::ExtensionType,
    test_case::test_case,
};
...
async fn fail_with_freeze_authority() {
    ...
    let wrong_mint = Keypair::new();
    let rent = banks_client.get_rent().await.unwrap();
    let mint_rent = rent.minimum_balance(lpl_token::state::Mint::LEN);
...
async fn success_with_supported_extensions() {
    let (mut banks_client, payer, recent_blockhash) = program_test().start().await;
    let stake_pool_accounts = StakePoolAccounts::new_with_token_program(lpl_token_2022::id());
...
async fn fail_with_fee_owned_by_wrong_token_program_id() {
    ...
    let account_rent = rent.minimum_balance(lpl_token::state::Account::LEN);
    let transaction = Transaction::new_signed_with_payer(
        &[system_instruction::create_account(
            &payer.pubkey(),
            &stake_pool_accounts.pool_fee_account.pubkey(),
            account_rent,
            lpl_token::state::Account::LEN as u64,
            &wrong_token_program.pubkey(),
        )],
        Some(&payer.pubkey()),
...
async fn fail_without_manager_signature() {
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(lpl_token::id(), false),
    ];
