...
use {
    ...
    lpl_stake_pool::{error::StakePoolError, id, instruction, state},
    lpl_token::error::TokenError,
    test_case::test_case,
};
...
async fn success_with_closed_manager_fee_account() {
    _success(lpl_token::id(), SuccessTestType::UninitializedManagerFee).await;
}
...
async fn fail_with_wrong_stake_program() {
    let (
        ...
        user_stake_recipient,
        tokens_to_burn,
    ) = setup_for_withdraw(lpl_token::id(), 0).await;
