...
use {
    ...
    lpl_token::{
        lumos_program::instruction::AccountMeta,
        state::{Account, Mint},
    },
    std::convert::TryInto,
};
...
fn process_init_reserve(
    ...
    lpl_token_init_account(TokenInitializeAccountParams {
    ...
    lpl_token_init_mint(TokenInitializeMintParams {
    ...
    lpl_token_transfer(TokenTransferParams {
    ...
    lpl_token_mint_to(TokenMintToParams {
        mint: reserve_collateral_mint_info.clone(),
...
fn process_redeem_reserve_collateral(
    ...
    lpl_token_burn(TokenBurnParams {
        mint: reserve_collateral_mint_info.clone(),
...
/// Issue a lpl_token `InitializeAccount` instruction.
#[inline(always)]
fn lpl_token_init_account(params: TokenInitializeAccountParams<'_>) -> ProgramResult {
    ...
    let ix = lpl_token::instruction::initialize_account(
        token_program.key,
        account.key,
        mint.key,
        owner.key,
...
/// Issue a lpl_token `InitializeMint` instruction.
#[inline(always)]
fn lpl_token_init_mint(params: TokenInitializeMintParams<'_, '_>) -> ProgramResult {
    ...
    let ix = lpl_token::instruction::initialize_mint(
        token_program.key,
        mint.key,
        authority,
        None,
...
#[inline(always)]
fn lpl_token_transfer(params: TokenTransferParams<'_, '_>) -> ProgramResult {
    ...
    let result = invoke_optionally_signed(
        &lpl_token::instruction::transfer(
            token_program.key,
            source.key,
            destination.key,
            authority.key,
...
fn lpl_token_mint_to(params: TokenMintToParams<'_, '_>) -> ProgramResult {
...
#[inline(always)]
fn lpl_token_burn(params: TokenBurnParams<'_, '_>) -> ProgramResult {
    let TokenBurnParams {
        mint,
        source,
        authority,
