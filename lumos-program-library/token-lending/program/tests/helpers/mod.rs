...
use {
    assert_matches::*,
    lumos_program::{program_option::COption, program_pack::Pack, pubkey::Pubkey},
    lumos_program_test::*,
    lumos_sdk::{
        account::Account,
...
pub fn add_lending_market(test: &mut ProgramTest) -> TestLendingMarket {
    ...
    let (lending_market_authority, bump_seed) =
        Pubkey::find_program_address(&[lending_market_pubkey.as_ref()], &lpl_token_lending::id());
...
pub fn add_reserve(
    ...
    let is_native = if liquidity_mint_pubkey == lpl_token::native_mint::id() {
        COption::Some(1)
    } else {
        COption::None
    };
...
pub async fn get_token_balance(banks_client: &mut BanksClient, pubkey: Pubkey) -> u64 {
    ...
    lpl_token::state::Account::unpack(&token.data[..])
        .unwrap()
        .amount
}
