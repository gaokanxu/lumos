use {
    ...
    log::*,
    lumos_clap_utils::input_parsers::{lamports_of_lum, value_of},
    lumos_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    lumos_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = lumos_cli_config::Config::default().keypair_path;

    lumos_logger::setup_with_default("lumos=info");
    lumos_metrics::set_panic_hook("faucet", /*version:*/ None);
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in LUM"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in LUM"),
        )
        .arg(
    ...
    let per_time_cap = lamports_of_lum(&matches, "per_time_cap");
    let per_request_cap = lamports_of_lum(&matches, "per_request_cap");
