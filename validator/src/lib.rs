...
pub use lumos_test_validator as test_validator;
use {
    console::style,
...
pub fn redirect_stderr_to_file(logfile: Option<String>) -> Option<JoinHandle<()>> {
    ...
    let filter = "lumos=info";
    match logfile {
        None => {
            lumos_logger::setup_with_default(filter);
            None
        }
        Some(logfile) => {
            #[cfg(unix)]
            ...
            #[cfg(not(unix))]
            {
                ...
                lumos_logger::setup_file_with_default(&logfile, filter);
                None
            }
        }
    }
