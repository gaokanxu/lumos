...
use {
    ...
    lumos_clap_utils::input_parsers::{keypair_of, keypairs_of, pubkey_of, value_of},
    lumos_core::{
    ...
    lumos_gossip::{cluster_info::Node, legacy_contact_info::LegacyContactInfo as ContactInfo},
    lumos_ledger::{
    ...
    lumos_perf::recycler::enable_recycler_warming,
    lumos_poh::poh_service,
    ...
    lumos_rpc_client_api::config::RpcLeaderScheduleConfig,
    lumos_runtime::{
    ...
    lumos_send_transaction_service::send_transaction_service,
    lumos_streamer::socket::SocketAddrSpace,
    lumos_tpu_client::tpu_client::DEFAULT_TPU_ENABLE_UDP,
    lumos_validator::{
        admin_rpc_service,
...
fn get_cluster_shred_version(entrypoints: &[SocketAddr]) -> Option<u16> {
    ...
    for entrypoint in entrypoints {
        match lumos_net_utils::get_cluster_shred_version(entrypoint) {
...
pub fn main() {
    ...
    let cli_app = app(lumos_version, &default_args);
        ...
        ("set-public-address", Some(subcommand_matches)) => {
            let parse_arg_addr = |arg_name: &str, arg_long: &str| -> Option<SocketAddr> {
                subcommand_matches.value_of(arg_name).map(|host_port| {
                    lumos_net_utils::parse_host_port(host_port).unwrap_or_else(|err| {
                        eprintln!(
    ...
    let logfile = {
        let logfile = matches
            .value_of("logfile")
            .map(|s| s.into())
            .unwrap_or_else(|| format!("lumos-validator-{}.log", identity_keypair.pubkey()));
    ...
    info!("{} {}", crate_name!(), lumos_version);
    info!("Starting validator with: {:#?}", std::env::args_os());

    let cuda = matches.is_present("cuda");
    if cuda {
        lumos_perf::perf_libs::init_cuda();
    ...
    let bind_address = lumos_net_utils::parse_host(matches.value_of("bind_address").unwrap())
        .expect("invalid bind_address");
    let rpc_bind_address = if matches.is_present("rpc_bind_address") {
        lumos_net_utils::parse_host(matches.value_of("rpc_bind_address").unwrap())
            .expect("invalid rpc_bind_address")
    } else if private_rpc {
        lumos_net_utils::parse_host("127.0.0.1").unwrap()
    } else {
        bind_address
    };
    ...
    let rpc_send_transaction_tpu_peers = matches
        ...
        .map(|values| {
            values
                .map(lumos_net_utils::parse_host_port)
    ...
    let mut validator_config = ValidatorConfig {
        ...
        rpc_config: JsonRpcConfig {
            ...
            faucet_addr: matches.value_of("rpc_faucet_addr").map(|address| {
                lumos_net_utils::parse_host_port(address).expect("failed to parse faucet address")
            }),
            full_api,
        ...
        rpc_addrs: value_t!(matches, "rpc_port", u16).ok().map(|rpc_port| {
            (
                ...
                // https://github.com/lumos-labs/lumos/issues/12250
            )
        }),
    ...
    let dynamic_port_range =
        lumos_net_utils::parse_port_range(matches.value_of("dynamic_port_range").unwrap())
    ...
    let public_rpc_addr = matches.value_of("public_rpc_addr").map(|addr| {
        lumos_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if !matches.is_present("no_os_network_limits_test") {
        if SystemMonitorService::check_os_network_limits() {
            ...
            eprintln!("OS network limit test failed. See: https://docs.lumoslabs.com/operations/guides/validator-start#system-tuning");
            exit(1);
        }
    }
        ...
        .unwrap_or_else(|| {
            if !entrypoint_addrs.is_empty() {
                ...
                let gossip_host = order.into_iter().find_map(|i| {
                    ...
                    lumos_net_utils::get_public_ip_addr(entrypoint_addr).map_or_else(
                        |err| {
                            eprintln!(
    ...
    let gossip_addr = SocketAddr::new(
        ...
        value_t!(matches, "gossip_port", u16).unwrap_or_else(|_| {
            lumos_net_utils::find_available_port_in_range(bind_address, (0, 1)).unwrap_or_else(
                |err| {
    ...
    let public_tpu_addr = matches.value_of("public_tpu_addr").map(|public_tpu_addr| {
        lumos_net_utils::parse_host_port(public_tpu_addr).unwrap_or_else(|err| {
    ...
    let public_tpu_forwards_addr =
        matches
            ...
            .map(|public_tpu_forwards_addr| {
                lumos_net_utils::parse_host_port(public_tpu_forwards_addr).unwrap_or_else(|err| {
    ...
    lumos_metrics::set_panic_hook("validator", Some(String::from(lumos_version)));
    lumos_entry::entry::init_poh();
