...
cat >/etc/apt/apt.conf.d/99-lumos <<'EOF'
...
"$LUMOS_ROOT"/net/scripts/install-docker.sh
usermod -aG docker "$SETUP_USER"
"$LUMOS_ROOT"/net/scripts/install-certbot.sh
...
"$LUMOS_ROOT"/net/scripts/install-earlyoom.sh
"$LUMOS_ROOT"/net/scripts/localtime.sh
"$LUMOS_ROOT"/net/scripts/install-rsync.sh
"$LUMOS_ROOT"/net/scripts/install-libssl-compatability.sh
