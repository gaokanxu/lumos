use {
    ...
    lumos_clap_utils::{self, input_parsers::*, keypair::*},
    lumos_cli_config::ConfigInput,
    lumos_cli_output::{
    ...
    lumos_vote_program::vote_state::VoteAuthorize,
    std::{
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} LUM)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} LUM)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} LUM) + fee ({1} LUM)")]
    InsufficientFundsForSpendAndFee(f64, f64, Pubkey),
    #[error(transparent)]
    InvalidNonce(lumos_rpc_client_nonce_utils::Error),
...
impl From<lumos_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: lumos_rpc_client_nonce_utils::Error) -> Self {
        match error {
            lumos_rpc_client_nonce_utils::Error::Client(client_error) => {
...
pub fn parse_command(
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            .gen_completions_to("lumos", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        ("deploy", Some(_matches)) => clap::Error::with_description(
            "`lumos deploy` has been replaced with `lumos program deploy`",
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of lumos-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Request an airdrop from Lumos Faucet;
...
mod tests {
    use {
        super::*,
        serde_json::json,
        lumos_rpc_client::mock_sender_for_cli::SIGNATURE,
        lumos_rpc_client_api::{
            request::RpcRequest,
            response::{Response, RpcResponseContext},
        },
        lumos_rpc_client_nonce_utils::blockhash_query,
        lumos_sdk::{
            pubkey::Pubkey,
            signature::{
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(lumos_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_process_command() {
        ...
        let mut config = CliConfig {
            ...
            json_rpc_url: "http://127.0.0.1:7777".to_string(),
            ..CliConfig::default()
        };
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 LUM");
        ...
        let new_authorized_pubkey = lumos_sdk::pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = lumos_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = lumos_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = lumos_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = lumos_sdk::pubkey::new_rand();
