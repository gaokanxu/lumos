use {
    ...
    lumos_rpc_client::rpc_client::{GetConfirmedSignaturesForAddress2Config, RpcClient},
    ...
    fn cluster_query_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("supply")
                .about("Get information about the cluster supply of LUM")
                .arg(
            ...
            SubCommand::with_name("rent")
                ...
                .arg(
                    Arg::with_name("lamports")
                        .long("lamports")
                        .takes_value(false)
                        .help("Display rent in lamports instead of LUM"),
                ),
        )
    }
}
...
pub fn process_catchup(
    ...
    loop {
        ...
        // especially if pointing to api.meinnet-beta.lumos.com at times
...
pub fn process_total_supply(rpc_client: &RpcClient, _config: &CliConfig) -> ProcessResult {
    ...
    Ok(format!("{} LUM", lamports_to_lum(supply.total)))
}
...
mod tests {
    use {
        ...
        lumos_sdk::signature::{write_keypair, Keypair},
        std::str::FromStr,
