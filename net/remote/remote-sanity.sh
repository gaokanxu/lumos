...
local|tar|skip)
  ...
  lumos_cli=lumos
  lumos_gossip=lumos-gossip
  lumos_install=lumos-install
  ;;
*)
...
(
  set -x
  $lumos_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $lumos_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: lumos-install test"
