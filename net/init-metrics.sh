...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and LUMOS_METRICS_CONFIG is
...
host="https://internal-metrics.lumos.com:8086"
...
if $useEnv; then
  [[ -n $LUMOS_METRICS_CONFIG ]] ||
    usage "LUMOS_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  LUMOS_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export LUMOS_METRICS_CONFIG=\"$LUMOS_METRICS_CONFIG\"" >> "$configFile"
