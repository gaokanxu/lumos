...
source "$LUMOS_ROOT"/scripts/configure-metrics.sh
...
LUMOS_CONFIG_DIR=$LUMOS_ROOT/config
...
_setup_secondary_mount() {
  ...
  (
    ...
    if [[ -d $SECONDARY_DISK_MOUNT_POINT ]] && \
      ...
      rm -rf "$LUMOS_CONFIG_DIR"
      ln -sfT $SECONDARY_DISK_MOUNT_POINT/config "$LUMOS_CONFIG_DIR"
    fi
  )
}
