...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "lumos", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The Lumos CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq, Eq)]
pub struct Config {
    /// The RPC address of a Lumos validator node.
    ///
    ...
    /// documentation for `lumos_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `lumos_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from Lumos addresses to human-readable names.
    ///
    ...
    /// `lumos_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "lumos", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.lumos.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.lumos.com"),
            "ws://api.devnet.lumos.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.lumos.com"),
            "wss://api.devnet.lumos.com/".to_string()
        );
