...
### Primary Validator
The following additional `lumos-validator` parameters are required to enable
...
following `lumos-validator` command-line argument changes:
...
```bash
$ lumos-validator wait-for-restart-window --identity validator-keypair.json \
  && lumos-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `lumos-validator set-identity
