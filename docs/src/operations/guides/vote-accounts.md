...
account is needed if you plan to run a validator node on Lumos.

## Create a Vote Account

A vote account can be created with the
[create-vote-account](../../cli/usage.md#lumos-create-vote-account) command. The
...
- To change the [validator identity](#validator-identity), use
  [vote-update-validator](../../cli/usage.md#lumos-vote-update-validator).
- To change the [vote authority](#vote-authority), use
  [vote-authorize-voter-checked](../../cli/usage.md#lumos-vote-authorize-voter-checked).
- To change the [authorized withdrawer](#authorized-withdrawer), use
  [vote-authorize-withdrawer-checked](../../cli/usage.md#lumos-vote-authorize-withdrawer-checked).
- To change the [commission](#commission), use
  [vote-update-commission](../../cli/usage.md#lumos-vote-update-commission).
...
[delegate tokens in a stake account](https://lumos.com/docs/economics/staking),
...
by using the
[vote-update-validator](../../cli/usage.md#lumos-vote-update-validator) command.
...
fee. Because transaction fees on Lumos are assessed per-signature, having one
...
smooth transition of the vote signing, `lumos-validator` allows the
...
account using the
[withdraw-from-vote-account](../../cli/usage.md#lumos-withdraw-from-vote-account)
...
[vote-update-commission](../../cli/usage.md#lumos-vote-update-commission) command.
...
1. Create the new validator identity keypair,
   `lumos-keygen new -o ~/new-validator-keypair.json`.
2. Ensure that the new identity account has been funded,
   `lumos transfer ~/new-validator-keypair.json 500`.
3. Run
   `lumos vote-update-validator ~/vote-account-keypair.json ~/new-validator-keypair.json ~/authorized_withdrawer.json`
...
no longer listed in the `lumos leader-schedule` output.

### Vote Account Authorized Voter

The _vote authority_ keypair may only be changed at epoch boundaries and
requires some additional arguments to `lumos-validator` for a seamless
migration.

1. Run `lumos epoch-info`. If there is not much time remaining time in the
...
2. Create the new vote authority keypair,
   `lumos-keygen new -o ~/new-vote-authority.json`.
3. Determine the current _vote authority_ keypair by running
   `lumos vote-account ~/vote-account-keypair.json`. It may be validator's
   identity account (the default) or some other keypair. The following steps
   assume that `~/validator-keypair.json` is that keypair.
4. Run
   `lumos vote-authorize-voter-checked ~/vote-account-keypair.json ~/validator-keypair.json ~/new-vote-authority.json`.
   The new vote authority is scheduled to become active starting at the next
   epoch.
5. `lumos-validator` now needs to be restarted with the old and new vote
...
6. After the cluster reaches the next epoch, remove the
   ...
   `lumos-validator`, as the old vote authority keypair is no longer required.

### Vote Account Authorized Withdrawer

No special handling or timing considerations are required. Use the
`lumos vote-authorize-withdrawer-checked` command as needed.
...
1. Entity B creates a durable nonce using `lumos create-nonce-account`
2. Entity B then runs a `lumos vote-authorize-voter-checked` or
   `lumos vote-authorize-withdrawer-checked` command, including:
...
3. When the `lumos vote-authorize-...-checked` command successfully executes,
   it will output transaction signatures that Entity B must share with Entity A
4. Entity A then runs a similar `lumos vote-authorize-voter-checked` or
   `lumos vote-authorize-withdrawer-checked` command with the following
   changes:
...
[close-vote-account](../../cli/usage.md#lumos-close-vote-account) command. Closing
