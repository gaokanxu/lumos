---
title: "Restarting a Lumos Cluster"
...
In Lumos 1.14 or greater, run the following command to output the latest
optimistically confirmed slot your validator observed:
```bash
lumos-ledger-tool -l ledger latest-optimistic-slots
```

In Lumos 1.13 or less, the latest optimistically confirmed can be found by looking for the more recent occurrence of
[this](https://github.com/lumos-labs/lumos/blob/0264147d42d506fb888f5c4c021a998e231a3e74/core/src/optimistic_confirmation_verifier.rs#L71)
metrics datapoint.
...
### Step 3. Optionally install the new lumos version

### Step 4. Create a new snapshot for slot `SLOT_X` with a hard fork at slot `SLOT_X`

```bash
$ lumos-ledger-tool -l <LEDGER_PATH> --snapshot-archive-path <SNAPSHOTS_PATH> --incremental-snapshot-archive-path <INCREMENTAL_SNAPSHOTS_PATH> create-snapshot SLOT_X <SNAPSHOTS_PATH> --hard-fork SLOT_X
```

The snapshots directory should now contain the new snapshot.
`lumos-ledger-tool create-snapshot` will also output the new shred version, and bank hash value,
...
> Steps:
>
> 1. Install the v1.1.12 release: https://github.com/lumos-labs/lumos/releases/tag/v1.1.12
...
> ```
>
>      You can check for which slots your ledger has with: `lumos-ledger-tool -l path/to/ledger bounds`
>
...
> b. Ask it over RPC what slot it's on: `lumos --url http://127.0.0.1:8899 slot`. It should return `SLOT_X` until we get to 80% stake
>
> Thanks!
...
```bash
$ lumos-ledger-tool -l ledger create-snapshot SLOT_X ledger --hard-fork SLOT_X \
