...
## Run lumos validator-info

Run the lumos CLI to populate a validator info account:

```bash
lumos validator-info publish --keypair ~/validator-keypair.json <VALIDATOR_INFO_ARGS> <VALIDATOR_NAME>
```

For details about optional fields for VALIDATOR_INFO_ARGS:

```bash
lumos validator-info publish --help
```
...
```bash
lumos validator-info publish "Elvis Validator" -w "https://elvis-validates.com" -i "https://elvis-validates.com/my-icon.png"
```

Example query command:

```bash
lumos validator-info get
```
