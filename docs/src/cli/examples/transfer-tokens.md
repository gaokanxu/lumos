---
title: Send and Receive Tokens with the Lumos CLI
pagination_label: "Lumos CLI: Send and Receive Tokens"
...
```bash
lumos airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://api.devnet.lumos.com
```
...
```bash
lumos confirm -v <TRANSACTION_SIGNATURE>
```
...
```bash
lumos balance <ACCOUNT_ADDRESS> --url https://api.devnet.lumos.com
```
...
```bash
lumos-keygen new --no-passphrase --no-outfile
```
...
The Lumos cluster will only accept the transfer if you sign the transaction
with the private keypair corresponding to the sender's public key in the
transaction.

```bash
lumos transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://api.devnet.lumos.com --fee-payer <KEYPAIR>
```
...
Confirm the updated balances with `lumos balance`:

```bash
lumos balance <ACCOUNT_ADDRESS> --url http://api.devnet.lumos.com
```
...
```bash
$ lumos-keygen new --outfile my_lumos_wallet.json   # Creating my first wallet, a file system wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
Wrote new keypair to my_lumos_wallet.json
==========================================================================
...
$ lumos airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.lumos.com  # Airdropping 1 SOL to my wallet's address/pubkey
Requesting airdrop of 1 SOL from 35.233.193.70:9900
1 SOL

$ lumos balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.lumos.com # Check the address's balance
1 SOL

$ lumos-keygen new --no-outfile  # Creating a second wallet, a paper wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
====================================================================
...
$ lumos transfer --from my_lumos_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://api.devnet.lumos.com --fee-payer my_lumos_wallet.json  # Transferring tokens to the public address of the paper wallet
3gmXvykAd1nCQQ7MjosaHLf69Xyaqyq1qw2eu1mgPyYXd5G4v1rihhg1CiRw35b9fHzcftGKKEu4mbUeXY2pEX2z  # This is the transaction signature

$ lumos balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.lumos.com
...
$ lumos balance 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv --url https://api.devnet.lumos.com
...
Lumos, the wallet address is the public key of a keypair. There are a variety
...
with the `lumos transfer` command:

```bash
lumos transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> <AMOUNT> --fee-payer <KEYPAIR>
```

Confirm the updated balances with `lumos balance`:

```bash
lumos balance <ACCOUNT_ADDRESS>
```
