    ...
    quote! {
        ...
        mod #test_mod_ident {
            ...
            fn test_abi_digest() {
                ...
                if ::std::env::var("LUMOS_ABI_BULK_UPDATE").is_ok() {
                    if #expected_digest != actual_digest {
                        #p!("sed -i -e 's/{}/{}/g' $(git grep --files-with-matches frozen_abi)", #expected_digest, hash);
                    }
                    ::log::warn!("Not testing the abi digest under LUMOS_ABI_BULK_UPDATE!");
                } else {
                    if let Ok(dir) = ::std::env::var("LUMOS_ABI_DUMP_DIR") {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Examine the diff in LUMOS_ABI_DUMP_DIR!: \n$ diff -u {}/*{}* {}/*{}*", dir, #expected_digest, dir, actual_digest);
                    } else {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Confirm the diff by rerunning before and after this test failed with LUMOS_ABI_DUMP_DIR!");
                    }
                }
            }
        }
