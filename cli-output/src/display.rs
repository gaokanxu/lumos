...
pub fn build_balance_message_with_config(
    ...
    let value = if config.use_lamports_unit {
        ...
        let sol = lamports_to_lum(lamports);
...
fn write_instruction<'a, W: io::Write>(
    ...
    if let AccountKeyType::Known(program_pubkey) = program_pubkey {
        if program_pubkey == &lumos_vote_program::id() {
        ...
        } else if program_pubkey == &lumos_sdk::system_program::id() {
            if let Ok(system_instruction) = limited_deserialize::<
                lumos_sdk::system_instruction::SystemInstruction,
            >(&instruction.data)
            {
...
fn write_fees<W: io::Write>(w: &mut W, transaction_fee: u64, prefix: &str) -> io::Result<()> {
    writeln!(w, "{}  Fee: ◎{}", prefix, lamports_to_lum(transaction_fee))
}
...
mod test {
    use {
        ...
        lumos_transaction_status::{Reward, RewardType, TransactionStatusMeta},
        std::io::BufWriter,
    };
